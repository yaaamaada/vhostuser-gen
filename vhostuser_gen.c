...
mygen_tx_loop(void)
{
	...
	while (!force_quit) {
		...
		if (rte_pktmbuf_alloc_bulk(l2fwd_pktmbuf_pool, bufs, MAX_PKT_BURST)) {
		...
		if (unlikely(nb_tx < MAX_PKT_BURST)) {
			rte_pktmbuf_free_bulk(&bufs[nb_tx], MAX_PKT_BURST - nb_tx);
		}
	}
}
...
mygen_rx_loop(void)
{
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {// ports per lcore
...
static int
mygen_launch_one_lcore(__rte_unused void *dummy)
{
	unsigned lcore_id;
	struct lcore_queue_conf *qconf;
	unsigned portid, dst_port;
	...
	if (portid == 1) {
		// rx
		printf("RX: lcore_id=%u\n", lcore_id);
		fflush(stdout);
		mygen_rx_loop();
	//} else {
	} else if (portid == 0) {
		// tx
		printf("TX: lcore_id=%u\n", lcore_id);
		fflush(stdout);
		mygen_tx_loop();
	}
	return 0;
...
main(int argc, char **argv)
{
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		while (rte_lcore_is_enabled(rx_lcore_id) == 0 ||
			...
			if (rx_lcore_id >= RTE_MAX_LCORE)
				rte_exit(EXIT_FAILURE, "Not enough cores\n");// loop until exceeding RTE_MAX_LCORE
		}
		...
		printf("Lcore %u: RX port %u TX port %u\n", rx_lcore_id,
		       portid, l2fwd_dst_ports[portid]);// the num of port is 2 -> 2 lcores needed
	}
	...
	rte_eal_mp_remote_launch(mygen_launch_one_lcore, NULL, CALL_MAIN);
